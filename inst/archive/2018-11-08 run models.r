...
save(var_day.sex.age, file = paste0(getwd(), "/models/var_day.sex.age_F.RData"))
...
#Table S1: variable ~ day.sex.age; intercept calculated ####
runthrough = c("time_to_sunrise_min", "time_to_sunset_min")

var_day.sex.age_T = list(
  lapply(runthrough, FUN = function(variables, data) gamm4.2(response = variables[1], explanatory = 'sex_age', by_gamm = "sex2", data = data, rm.intercept = FALSE, smooth_over = 'YDAY' ), data = subset(x2, YDAY < 90)),
  lapply(runthrough, FUN = function(variables, data) gamm4.2(response = variables[1], explanatory = 'sex_age', by_gamm = "sex2", data = data, rm.intercept = FALSE, smooth_over = 'rel_day' ), data = subset(x2, rel_day >= -21))
)
save(var_day.sex.age_T, file = paste0(getwd(), "/models/var_day.sex.age_T.RData"))
