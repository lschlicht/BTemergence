...
plot_descriptive_figure(modellist_yday = list(ss_m1, ss_f1),
                        ...
                        ylim_boxplot = c(-150, 30), ylim_gammplot = c(-60, 10))
dev.off()
...
create_gamm_plot(var = "daylength", data = subset(x2, YDAY < 90), modelset = list(act_m1, act_f1) , ylab = list("Hours of activity", cex = 2), COL = c(MALE_COLOUR, FEMALE_COLOUR), xlim = c(-97, 90), xlab = "Date (1=1st Jan.)", ylim = c(8, 16), se = 1.96)
axis(1, las = 0)
...
ENV = unique(ENV)
ENV = dcast(ENV, YDAY ~ ., value.var = c("avgTemperature", "precipitation_dawn", "precipitation_dusk"), fun = mean, na.rm = TRUE)
W_ENV = copy(ENV)#next, define half-months (!+ two week intervals)
W_ENV[YDAY %in% c(-124:-109), YY := mean(c(-124, -109))]
W_ENV[YDAY %in% c(-108:-94), YY := mean(c(-108,-94 ))]
W_ENV[YDAY %in% c(-93:-78), YY := mean(c(-93,-78))]
W_ENV[YDAY %in% c(-77:-62), YY := mean(c(-77,-62))]
W_ENV[YDAY %in% c(-61:-46), YY := mean(c(-61,-46))]
W_ENV[YDAY %in% c(-45:-31), YY := mean(c(-45,-31))]
W_ENV[YDAY %in% c(-30:-15), YY := mean(c(-30,-15))]
W_ENV[YDAY %in% c(-14:0), YY := mean(c(-14,0))]
W_ENV[YDAY %in% c(1:16), YY := mean(c(1,16))]
W_ENV[YDAY %in% c(17:31), YY := mean(c(17,31))]
W_ENV[YDAY %in% c(32:46), YY := mean(c(32,46))]
W_ENV[YDAY %in% c(47:60), YY := mean(c(47,60))]
W_ENV[YDAY %in% c(61:76), YY := mean(c(61,76))]
W_ENV[YDAY %in% c(77:92), YY := mean(c(77,92))]
W_ENV[YDAY %in% c(93:108), YY := mean(c(93,108))]
W_ENV[YDAY %in% c(109:123), YY := mean(c(109,123))]
W_ENV[YDAY %in% c(124:139), YY := mean(c(124,139))]
W_ENV[YDAY %in% c(140:155), YY := mean(c(140,155))]

W_ENV = dcast(W_ENV, YY ~ ., value.var = c("avgTemperature", "precipitation_dawn", "precipitation_dusk"), fun = function(x) round(mean(x, na.rm = TRUE), digits = 1))
...
plot(x = ENV[, YDAY], y = ENV[, avgTemperature], type = 'l', xlim = c(-100, 150), xaxt = "n", xlab = '', ylab = "Temperature", yaxt = 'n')
rect(c(-94, -31, 31, 92, 155)-0.5, rep(-100,5), c(-61, 1, 61, 124, 187)+0.5, rep(100, 5), border = NA, col = "light grey")
par(new = TRUE)
plot(x = ENV[, YDAY], y = ENV[, avgTemperature], type = 'l', xlim = c(-100, 150), xaxt = "n", xlab = '', ylab = "Temperature", yaxt = 'n')
text(W_ENV[, YY], 20, W_ENV[, avgTemperature], cex = 4)

axis(1, labels = FALSE, lwd = 5)
axis(2, lwd = 5)

par(mar = c(6.5,16.9,6.5,0.5))
plot(x = ENV[, YDAY], y = ENV[, precipitation_dawn], type = 'l', xlim = c(-100, 150), xaxt = 'n', ylab = "Rain at dawn", xlab = '', yaxt = 'n')
axis(1, labels = FALSE, lwd = 5)
axis(2, lwd = 5)

par(mar = c(12.9,16.9,0.1, 0.5))
plot(x = ENV[, YDAY], y = ENV[, precipitation_dusk], type = 'l', xlim = c(-100, 150), ylab = "Rain at dusk", xlab = 'Day of season (1=1st Jan)', xaxt = 'n', yaxt = 'n')
axis(1, lwd = 5)
axis(2, lwd = 5)
dev.off()
...
par(las = 1)
  ...
  plot(yf, type = "h", ylim = YLIM, xlim = XLIM, xaxt = 'n', ylab = list("Females", cex = 1.3), col = FEMALE_COLOUR, xlab = list("Day of season (1=1st Jan)", cex = 1.2))
  axis(1, at = c(-200, -150, -100, -50, 0, 50), labels = c(-200, -150, -100, -50, 0, 50))
  par(mar = c(5.1, 0.1, 0.1, 4.1))
  plot(af, type = "h", ylim = YLIM, xlim = XLIM, xaxt = 'n', yaxt = 'n', col = FEMALE_COLOUR, xlab = list("Day of season (1=1st Jan)", cex = 1.2))
  axis(1, at = c(-200, -150, -100, -50, 0, 50), labels = c(-200, -150, -100, -50, 0, 50))
...
#Figure S3: sample sizes rel_day####
  ...
  par(mar = c(5.1, 4.1, 0.1, 0.1))
  plot(yf, type = "h", ylim = YLIM, xlim = XLIM, xaxt = 'n', ylab = list("Females", cex = 1.3), col = FEMALE_COLOUR, xlab = list("Days to first egg", cex = 1.2))
  axis(1, at = c(-200, -150, -100, -50, 0, 50), labels = c(-200, -150, -100, -50, 0, 50))
  par(mar = c(5.1, 0.1, 0.1, 4.1))
  plot(af, type = "h", ylim = YLIM, xlim = XLIM, xaxt = 'n', yaxt = 'n', col = FEMALE_COLOUR, xlab = list("Days to first egg", cex = 1.2))
  axis(1, at = c(-200, -150, -100, -50, 0, 50), labels = c(-200, -150, -100, -50, 0, 50))
...
year_ID = subset(x, rel_day %in% -2:0, select = c('breeding_season', 'ID', 'sex', 'age', 'yid'))
year_ID[, count := length(sex), by = list(ID, breeding_season)]
year_ID = unique(year_ID)
year_ID = subset(year_ID, count == 3)


year_ID = subset(x, rel_day >= -50 & rel_day < 50 & yid %in% year_ID[,yid], select = c('breeding_season', 'ID', 'sex', 'age'))
year_ID[, count := length(sex), by = list(ID, breeding_season)]
year_ID = unique(year_ID)
...
plot_individuals(x, i = 1, var = VAR, xaxt = 'n', ylab = '', xlab= '', xlim = XLIM, ylim = YLIM, lty = LTY, cex = 2, plot.lines = PLOT.LINES)
legend(-218, 70, "yearling male (2016)", bty = 'n', cex = cex.legend)

par(mar = c(mar.bottom/3, mar.left, 2*(mar.bottom/3), 0.2))
plot_individuals(x, i = 2, var = VAR, xaxt = 'n', ylab = '', xlab= '', xlim = XLIM, ylim = YLIM, lty = LTY, cex = 2, plot.lines = PLOT.LINES)
legend(-218, 70, "adult male (2015)", bty = 'n', cex = cex.legend)
text(25, 45, "98.88", cex = 3, adj = 0)
text(-50, 45, "179.17", cex = 3, adj = 1)
arrows(23, 45, 15, 60)
arrows(-50, 45, -42, 60)

par(mar = c(2*(mar.bottom)/3, mar.left, mar.bottom/3, 0.2))
plot_individuals(x, i = 3, var = VAR, xaxt = 'n', ylab = '', xlab = '', xlim = XLIM, ylim = YLIM, lty = LTY, cex = 2, plot.lines = PLOT.LINES)
legend(-218, 70, "yearling female (2015)", bty = 'n', cex = cex.legend)
text(-60, 45, "179.65", cex = 3, adj = 1)
arrows(-60, 45, -52, 60)

par(mar = c(mar.bottom, mar.left, 0.2, 0.2))
plot_individuals(x, i = 4, var = VAR, ylab = '', xlab = XLAB, xlim = XLIM, ylim = YLIM, lty = LTY, cex = 2, plot.lines = PLOT.LINES)
...
par(mar = c(0.1, mar.left, mar.bottom, 0.2))
plot_individuals(x, i = 1, var = VAR, xaxt = 'n', xlab = '', ylab = '', xlim = XLIM, ylim = YLIM, lty = LTY, cex = 2, plot.lines = PLOT.LINES)
legend(-218, 70, "yearling male (2016)", bty = 'n', cex = cex.legend)

par(mar = c(mar.bottom/3, mar.left, 2*(mar.bottom/3), 0.2))
plot_individuals(x, i = 2, var = VAR, xaxt = 'n', xlab = '', ylab = '', xlim = XLIM, ylim = YLIM, lty = LTY, cex = 2, plot.lines = PLOT.LINES)
legend(-218, 70, "adult male (2015)", bty = 'n', cex = cex.legend)

par(mar = c(2*(mar.bottom)/3, mar.left, mar.bottom/3, 0.2))
plot_individuals(x, i = 3, var = VAR, xaxt = 'n', xlab = '', ylab = '', xlim = XLIM, ylim = YLIM, lty = LTY, cex = 2, plot.lines = PLOT.LINES)
legend(-218, 70, "yearling female (2015)", bty = 'n', cex = cex.legend)

par(mar = c(mar.bottom, mar.left, 0.2, 0.2))
plot_individuals(x, i = 4, var = VAR, xlab = XLAB, ylab = '', xlim = XLIM, ylim = YLIM, lty = LTY, cex = 2, plot.lines = PLOT.LINES)
legend(-218, 70, "adult female (2017)", bty = 'n', cex = cex.legend)
...
plot_seasonal_progress(tmp1 = list(tmp1[[1]], tmp1[[2]]), tmp2 = list(tmp2[[1]], tmp2[[2]]), YLIM = c(-0.3, 0.2), XLAB = "Onset of activity", YLAB = "avg. Temperature\r\n")
dev.off()

jpeg(file = paste0(getwd(), "/figures/S7_Temperature and cessation of activity across year.jpg"), width = 900, height = 900)
plot_seasonal_progress(tmp1 = list(tmp1[[3]], tmp1[[4]]), tmp2 = list(tmp2[[3]], tmp2[[4]]), YLIM = c(-0.1, 0.4), XLAB = "Cessation of activity", YLAB = "avg. Temperature\r\n")
dev.off()


jpeg(file = paste0(getwd(), "/figures/S8_Rainfall and onset of activity across year.jpg"), width = 900, height = 900)
plot_seasonal_progress(tmp1 = list(tmp1[[5]], tmp1[[6]]), tmp2 = list(tmp2[[5]], tmp2[[6]]), YLIM = c(-0.05, 0.45), XLAB = "Onset of activity", YLAB = "avg. rainfall at dawn\r\n")
dev.off()

jpeg(file = paste0(getwd(), "/figures/S9_Rainfall and cessation of activity across year.jpg"), width = 900, height = 900)
plot_seasonal_progress(tmp1 = list(tmp1[[7]], tmp1[[8]]), tmp2 = list(tmp2[[7]], tmp2[[8]]), YLIM = c(-0.3, 0.2), XLAB = "Cessation of activity", YLAB = "avg. rainfall at dusk\r\n")
dev.off()
