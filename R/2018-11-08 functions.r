...
create_gamm_plot = function(var, data = x2, include_age = FALSE, modelset = list(), COL = c("red", "blue"), plot_sex = NULL, plot_which = NULL, ...) {

  SHIFT1 = summary(modelset[[1]]$gam)$p.table[1,1]#  + mean(plot(modelset[[1]]$gam, select = 0)[[1]]$fit)
  SHIFT2 = summary(modelset[[2]]$gam)$p.table[1,1]# + mean(plot(modelset[[2]]$gam, select = 0)[[1]]$fit)

  #plot
  plot.gam(modelset[[1]]$gam, shift = SHIFT1, col = COL[1], select = 1, rug = FALSE, xaxt = 'n', ...)
...
plot_descriptive_figure = function(modellist_yday, modellist_rel_day, d_list, out_list, YLAB, VAR,ylim_boxplot = c(-60, 70), ylim_gammplot = c(-40, 20)) {
    ...
    #plot
    boxplot_emergence(d1 = d_list[[1]], d2 = out_list[[1]], xaxt = 'n', xlab = '', ylab = list('Males', cex = cex.sex), col = MALE_COLOUR, xlim = c(-100, 90), cex = cex.pt, ylim = ylim_boxplot)
    ...
    boxplot_emergence(d1 = d_list[[3]], d2 = out_list[[3]], xaxt = 'n', xlab = '', ylab = list('Females', cex = cex.sex), col = FEMALE_COLOUR, xlim = c(-100, 90), cex = cex.pt, ylim = ylim_boxplot)
    ...
    create_gamm_plot(var = VAR, modelset = modellist_yday , ylab = list(""), COL = c(MALE_COLOUR, FEMALE_COLOUR), xlim = c(-100, 90), xlab = list("Day of season (1=1st Jan.)"), ylim = ylim_gammplot, se = 1.96)
    rect(c(-94, -31, 59)-0.5, rep(-100,3), c(-63, -1, 31)+0.5, rep(100, 3), border = NA, col = "light grey")
    par(new = TRUE)
    create_gamm_plot(var = VAR, modelset = modellist_yday , ylab = list(""), COL = c(MALE_COLOUR, FEMALE_COLOUR), xlim = c(-100, 90), xlab = list("Day of season (1=1st Jan.)"), ylim = ylim_gammplot, se = 1.96)
    axis(1, las = 0)
    ...
    rect(c(-14, 0, 14, 28, 42, 56)-0.5, rep(-100,6), c(-8, 6, 20, 34, 48, 62)+0.5, rep(100, 6), border = NA, col = "light grey")
    par(new = TRUE)
    create_gamm_plot(var = VAR, modelset = modellist_rel_day , ylab = list(""), COL = c(MALE_COLOUR, FEMALE_COLOUR), xlim = c(-21, 60), xlab = list("Days to first egg"), yaxt = 'n', ylim = ylim_gammplot, se = 1.96)
    axis(1, las = 0)
    axis(3, at = c(0, 10, 24, 44), labels = rep('', 4), tcl = tcl.stage2)
...
plot_individuals = function(data = x, i = 1, yead_ID = year_ID, var = "time_to_sunrise_min", MALE_COLOUR = "#5000FFFF", FEMALE_COLOUR = "#FF758AFF", plot.lines = FALSE, ...) {
...
plot_seasonal_progress = function(tmp1, tmp2, YLIM = c(-10, 5), XLAB = "Onset of activity", YLAB = paste0("average Temperature\r\n", ifelse(i == 1, "males", "females"))) {
  par(mfrow = c(2,1))
  ...
  for(i in 1 : 2) {
    ...
    plot(c(-4, 0) ~ c(1,3), type = 'n', ylim = YLIM, xlim = c(-2.5, 7.5), ylab = paste0("Est. ", YLAB2), xaxt = 'n', xlab = ifelse(i == 1, "", XLAB))
